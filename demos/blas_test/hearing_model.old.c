...
Connections *bias_phoclean,*bias_semclean;
...
void build_hearing_model(int samples)
{
  ...
  for(i=0;i<psh->numUnits;i++)
    bias_psh->weights[i][0]= -3.0;

  for(i=0;i<sph->numUnits;i++)
    bias_sph->weights[i][0]= -3.0;

  for(i=0;i<pho_cleanup->numUnits;i++)
    bias_phoclean->weights[i][0]= -3.0;

  for(i=0;i<sem_cleanup->numUnits;i++)
    bias_semclean->weights[i][0]= -3.0;
